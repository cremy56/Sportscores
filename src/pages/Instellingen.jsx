...
export default function Instellingen() {
    ...
    const activeItem = settingsMenu.find(item => location.pathname === item.to || (item.to !== '/instellingen' && location.pathname.startsWith(item.to)));
    const pageTitle = activeItem ? activeItem.name : 'Instellingen';

    return (
        // --- START WIJZIGING: Wrapper toegevoegd voor consistente layout ---
        <div className="fixed inset-0 bg-slate-50 overflow-y-auto">
            <div className="max-w-7xl mx-auto px-4 pt-20 pb-6 lg:px-8">
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <main className="flex-1 min-w-0">
                        <h2 className="text-2xl font-bold text-gray-800 mb-6 hidden lg:block">{pageTitle}</h2>
                        <Outlet />
                    </main>
                </div>
            </div>
