// src/pages/Evolutie.jsx - Mobile Optimized with Centered Cards
            ...
            <div className="flex justify-center w-full">
                <div className="w-full max-w-5xl"> {/* Beperkte breedte voor centrering */}
                    <div className="bg-white/60 backdrop-blur-lg rounded-xl sm:rounded-2xl shadow-xl border border-white/30 min-h-[75vh] p-2 sm:p-4 lg:p-6"> {/* Hogere minimum hoogte */}   
                        ...
                        {/* Evolution Cards - GECENTREERD EN RESPONSIVE */}
                        {!loading && !error && selectedStudent && Object.keys(grouped_data).length > 0 && (
                            <>
                                {/* SINGLE COLUMN LAYOUT VOOR PERFECTE CENTRERING */}
                                <div className="flex flex-col items-center gap-6 sm:gap-8">
                                    {Object.entries(grouped_data).map(([categoryName, testsInCategory]) => (
                                        <div key={`${categoryName}-${selectedYear}`} className="w-full max-w-2xl">
                                            <EvolutionCard
