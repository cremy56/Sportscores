...
const ProfileMenu = ({
  ...
  const handleRoleChange = (newRole) => {
    ...
    const restrictedPaths = {
      'leerling': ['/instellingen', '/ehbo-monitor', '/groepsbeheer', '/sporttesten'],
...
export default function Layout({ profile, school, selectedStudent, setSelectedStudent, activeRole, setActiveRole }) {
  ...
  useEffect(() => {
    ...
    const currentPath = location.pathname;
    const restrictedPaths = {
      'leerling': ['/instellingen', '/gebruikersbeheer', '/trainingsbeheer', '/schoolbeheer', '/ehbo-monitor', '/groepsbeheer', '/sporttesten'],
    ...
    // (Welzijn-guard verwijderd bij ontmanteling welzijnsmodule jul 2026 —
    //  de /gezondheid- en /welzijnsmonitor-routes zijn nu redirects in App.jsx.)
    if (isPathRestricted) {
      navigate('/');
    }
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md shadow-sm border-b border-white/20">
        <nav className="relative w-full px-4 md:px-8 py-2 flex items-center justify-between">
          ...
          <ul className="hidden md:flex items-center space-x-6 flex-grow justify-center">
            ...
            {isTeacherOrAdmin && (
              <li><NavLink to="/ehbo-monitor" className={({ isActive }) => (isActive ? activeLinkStyle : inactiveLinkStyle)}>EHBO Monitor</NavLink></li>
            )}
