...
// ─── Hashing helpers ──────────────────────────────────────────────────────────
// h1 (huidig): HMAC-SHA256 met geheime pepper uit Secret Manager (HASH_PEPPER_*).
//   Zonder de pepper is brute-forcen van (raadbare) Smartschool-ID's onmogelijk;
//   kale SHA-256 was daar kwetsbaar voor (DPO-opmerking, jun 2026).
//   De pepper komt via getHashPepper() uit lib/keyManager.js en leeft ENKEL
//   server-side — nooit client-side hashen.
// legacy: kale SHA-256 — enkel nog voor het migratiescript
//   (api/admin/migrate-hash.js) en de tijdelijke login-fallback in api/auth.js.
//   Verwijderen zodra de migratie afgerond en geverifieerd is.
export const CURRENT_HASH_VERSION = 'h1';

export const generateHash = (smartschoolUserId, pepper) => {
    if (!smartschoolUserId) throw new Error('smartschoolUserId is verplicht voor hash-generatie');
    if (!pepper) throw new Error('Hash pepper ontbreekt — geef het resultaat van getHashPepper() mee');
    return CryptoJS.HmacSHA256(smartschoolUserId, pepper).toString();
};

export const generateLegacyHash = (smartschoolUserId) => {
