...
export function valideerAvatar(avatar) {
  ...
  // Optionele lichaamskeuze — alléén relevant als het profiel-geslacht 'X' of
  // onbekend is (de handler beslist of hij bewaard wordt).
  if (avatar.lichaam !== undefined) {
    if (!['m', 'v', 'neutraal'].includes(avatar.lichaam)) return null;
    schoon.lichaam = avatar.lichaam;
  }
  return schoon;
}

// ─── Weergave-afleidingen (niet opgeslagen — hergebruik van profieldata) ──────
// Zelfde logica als src/data/sportbuddy/sporten.js — synchroon houden.
