...
// opties.heet/koud/matchdag/blessure: contextmodifiers — de juiste keuze
// verschilt per dag (anti-repetitie by design).
const KRACHT_TRAINBAARHEID = { 1: 0.35, 2: 0.7, 3: 1.0 };

export function verwerkDag(state, keuzes, opties = {}) {
  const graad = opties.graad || 2;
  const { heet = false, koud = false, matchdag = false, blessure = false } = opties;
  ...
  // ── Water: de eis hangt af van het WEER (hitte = meer nodig) ────────────────
  if (heet) {
    if (water.type === 'weinig') {
      nieuw.vermoeidheid += 9;
      meldingen.push('Te weinig gedronken op een hittedag: je buddy raakt gedehydrateerd — hartslag omhoog, prestatie omlaag.');
    } else if (water.type === 'voldoende') {
      nieuw.vermoeidheid += 3;
      meldingen.push('Op zo\'n warme dag is "voldoende" eigenlijk te weinig: bij hitte verliest een sporter tot een liter zweet per uur extra.');
    } else {
      meldingen.push('Slim: extra drinken bij hitte houdt het koelsysteem van je buddy draaiende.');
    }
  } else if (water.type === 'weinig') {
  ...
  // ── Kou: koude spieren zijn kwetsbare spieren ───────────────────────────────
  if (koud && belasting >= 35) {
    nieuw.vermoeidheid += 2;
    meldingen.push('Trainen in de kou: zorg voor een extra lange warming-up — koude spieren raken sneller geblesseerd.');
  }

  // ── Blessure: forceren heeft een prijs ──────────────────────────────────────
  if (blessure && belasting >= 45) {
    nieuw.vermoeidheid += 5;
    meldingen.push('Zwaar trainen mét een blessure: je buddy compenseert en overbelast de rest van zijn lichaam. Hersteltijd respecteren!');
  }

  // ── Matchdag ────────────────────────────────────────────────────────────────
  if (matchdag && keuzes.training === 'match') {
    meldingen.push('Wedstrijd gespeeld! Hoe je buddy presteerde, hangt af van de hele week: vorm, voeding, slaap en stress.');
  } else if (matchdag && belasting < 70) {
    meldingen.push('Het was wedstrijddag, maar je buddy stond niet aan de aftrap. Volgende week een nieuwe kans.');
  }
