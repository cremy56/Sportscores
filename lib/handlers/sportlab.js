...
function isBinnenSchoolUren() {
    // NB: superadmin-uitzondering gebeurt op de call-site (isSuperAdmin-check)
...
// ─── HELPER: toernooi-autorisatie ─────────────────────────────────────────────
// Scores invoeren / rondes doorschuiven / toernooien starten mag door:
//   1. leerkracht/administrator/super-administrator van de EIGEN school
//   2. een leerling die in DEZE sessie de rol 'toernooileider' heeft
// Altijd in combinatie met een school-check op het object zelf.
async function magToernooiBeheren(sessieId, uid) {
    const userSnap = await db.collection('users').doc(uid).get();
    const rol = userSnap.data()?.rol;
    if (['leerkracht', 'administrator', 'super-administrator'].includes(rol)) return true;

    const deelname = await db.collection('sport_lab_deelnames')
        .where('sessie_id', '==', sessieId)
        .where('leerling_firebase_uid', '==', uid)
        .where('rol', '==', 'toernooileider')
        .limit(1)
        .get();
    return !deelname.empty;
}
...
export async function handleStartToernooi(req, res, decodedToken) {
    try {
        ...
        // 🔒 FIX (jul 2026): sessie bestond/school werd nooit geverifieerd en
        // input werd niet gevalideerd — toernooien konden aangemaakt worden
        // voor willekeurige sessie-ID's met willekeurige data.
        if (!['poule', 'king', 'knockout'].includes(type)) {
            return res.status(400).json({ error: 'Ongeldig toernooitype.' });
        }
        if (!Array.isArray(teams) || teams.length < 2 || teams.length > 20) {
            return res.status(400).json({ error: 'Tussen 2 en 20 teams vereist.' });
        }
        const veiligeTeams = teams.map((t, i) => ({
            id: String(t.id || `team_${i + 1}`).substring(0, 40),
            naam: String(t.naam || `Team ${i + 1}`).substring(0, 40),
            spelers: Array.isArray(t.spelers)
                ? t.spelers.slice(0, 20).map(sp => ({ naam: String(sp?.naam || '').substring(0, 50) }))
                : [],
        }));

        const sessieSnap = await db.collection('sport_lab_sessions').doc(sessieId).get();
        if (!sessieSnap.exists) return res.status(404).json({ error: 'Sessie niet gevonden.' });
        if (sessieSnap.data().school_id !== verifiedSchoolId) {
            return res.status(403).json({ error: 'Geen toegang.' });
        }
        if (!(await magToernooiBeheren(sessieId, decodedToken.uid))) {
            return res.status(403).json({ error: 'Enkel de leerkracht of de toernooileider kan een toernooi starten.' });
        }

        // Bepaal automatisch of de leerkracht zelf leidt:
        // zolang geen enkele leerling de rol 'toernooileider' heeft opgenomen,
        // voert de leerkracht zelf de uitslagen in (geen aparte leerling-evaluatie).
        const leiderSnap = await db.collection('sport_lab_deelnames')
            .where('sessie_id', '==', sessieId)
            .where('rol', '==', 'toernooileider')
            .limit(1)
            .get();
        const leerkrachtLeidt = leiderSnap.empty;
        ...
        if (type === 'poule') {
            ...
            let t = [...veiligeTeams].sort(() => Math.random() - 0.5); // Willekeurige start-loting
        ...
        const toernooiRef = await db.collection('sport_lab_toernooien').add({
            ...
            type: type,
            status: 'actief',
            leerkracht_leidt: leerkrachtLeidt,
            teams: veiligeTeams,
...
export async function handleUpdateMatchScore(req, res, decodedToken) {
    try {
        ...
        // 🔒 FIX (jul 2026): school-check op het toernooi zelf + autorisatie —
        // voorheen kon ELKE ingelogde gebruiker (ook van een andere school)
        // scores van elk toernooi aanpassen.
        if (toernooiSnap.data().school_id !== verifiedSchoolId) {
            return res.status(403).json({ error: 'Geen toegang.' });
        }
        if (!(await magToernooiBeheren(toernooiSnap.data().sessie_id, decodedToken.uid))) {
            return res.status(403).json({ error: 'Enkel de leerkracht of de toernooileider kan scores invoeren.' });
        }
...
export async function handleStopToernooi(req, res, decodedToken) {
    try {
        ...
        // 🔒 FIX (jul 2026): school-check — een leerkracht van school A kon
        // voorheen een toernooi van school B stoppen.
        const stopRef  = db.collection('sport_lab_toernooien').doc(toernooiId);
        const stopSnap = await stopRef.get();
        if (!stopSnap.exists) return res.status(404).json({ error: 'Toernooi niet gevonden.' });
        if (stopSnap.data().school_id !== verifiedSchoolId) {
            return res.status(403).json({ error: 'Geen toegang.' });
        }

        await stopRef.update({ status: 'gestopt' });
        return res.status(200).json({ success: true });
...
export async function handleVolgendeRonde(req, res, decodedToken) {
    try {
        ...
        // 🔒 FIX (jul 2026): school-check + autorisatie — voorheen kon iedere
        // ingelogde gebruiker rondes doorschuiven van elk toernooi.
        if (toernooi.school_id !== verifiedSchoolId) {
            return res.status(403).json({ error: 'Geen toegang.' });
        }
        if (!(await magToernooiBeheren(toernooi.sessie_id, decodedToken.uid))) {
            return res.status(403).json({ error: 'Enkel de leerkracht of de toernooileider kan de ronde doorschuiven.' });
        }
...
export async function handleGetWaarnemerMetingen(req, res, decodedToken) {
    try {
        ...
        let query;
        if (sessieId) {
            // 🔒 FIX (jul 2026): de sessie-tak verifieerde school noch eigenaar —
            // een leerkracht van een andere school kon met een sessie-ID metingen
            // (incl. ontsleutelde leerlingnamen) van elke sessie uitlezen.
            const sessieCheck = await db.collection('sport_lab_sessions').doc(sessieId).get();
            if (!sessieCheck.exists) return res.status(404).json({ error: 'Sessie niet gevonden.' });
            const sessieCheckData = sessieCheck.data();
            if (sessieCheckData.school_id !== verifiedSchoolId) {
                return res.status(403).json({ error: 'Geen toegang.' });
            }
            const callerRol = callerSnap.data()?.rol;
            if (sessieCheckData.leerkracht_id !== decodedToken.uid
...
export async function handleVerwijderWaarnemerMetingen(req, res, decodedToken) {
    try {
        ...
        // 🔒 FIX (jul 2026): school-check — admin van school A kon een meting
        // van school B verwijderen.
        if (docData.school_id !== verifiedSchoolId) {
            return res.status(403).json({ error: 'Geen toegang.' });
        }
...
export async function handleMarkeerWaarnemerGekoppeld(req, res, decodedToken) {
    try {
        ...
        // 🐛 FIX (jul 2026): XP volgde niet het volledige patroon — periode- en
        // schooljaar-tellers en het xp_transactions-record ontbraken, waardoor
        // waarnemer-XP niet meetelde in rankings en onzichtbaar was in de historie.
        const batch = db.batch();
        batch.update(ref, { status: 'gekoppeld', gekoppeld_op: Timestamp.now() });

        if (docData.ingediend_door) {
            const waarnemerRef = db.collection('users').doc(docData.ingediend_door);
            batch.update(waarnemerRef, {
                xp: FieldValue.increment(XP.ZELFREFLECTIE),
                xp_current_period: FieldValue.increment(XP.ZELFREFLECTIE),
                xp_current_school_year: FieldValue.increment(XP.ZELFREFLECTIE),
                last_activity_date: new Intl.DateTimeFormat('en-CA', { timeZone: 'Europe/Brussels' }).format(new Date()),
                last_activity: Timestamp.now(),
            });
            batch.set(waarnemerRef.collection('xp_transactions').doc(), {
                amount: XP.ZELFREFLECTIE,
                reason: 'sportlab_waarnemer',
                source_id: metingId,
                metadata: { sessie_id: docData.sessie_id, sport_type: docData.sport_type || null },
                created_at: Timestamp.now(),
            });
        }
