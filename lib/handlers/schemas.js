...
export async function handleAddOptioneelSchema(req, res, decodedToken) {
    try {
        const { leerlingId, schemaId: schemaIdRaw, schemaTemplateId, schoolId } = req.body;
        const schemaId = schemaIdRaw || schemaTemplateId;  // client stuurt beide namen
        const verifiedSchoolId = await getSchoolId(decodedToken.uid);
        if (schoolId !== verifiedSchoolId) return res.status(403).json({ error: 'Geen toegang.' });
...
export async function handleStartSchema(req, res, decodedToken) {
    try {
        const { leerlingId, schemaId: schemaIdRaw, schemaTemplateId, type, schoolId } = req.body;
        const schemaId = schemaIdRaw || schemaTemplateId;  // client stuurt beide namen
        const verifiedSchoolId = await getSchoolId(decodedToken.uid);
        if (schoolId !== verifiedSchoolId) return res.status(403).json({ error: 'Geen toegang.' });

        // Zonder geldige id werd de docId "..._undefined" en faalde de write
        // met een 500 (Firestore weigert schema_id: undefined). Nette 400.
        if (!leerlingId || !schemaId) {
            return res.status(400).json({ error: 'leerlingId en schemaId (of schemaTemplateId) zijn verplicht.' });
        }
