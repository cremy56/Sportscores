...
function metDagstaat(buddy) {
    return { ...buddy, dagstaat: afgeleiden(buddy) };
}
...
export async function handleCreateBuddy(req, res, decodedToken) {
    ...
    try {
        ...
        return res.status(200).json({ success: true, buddy: metDagstaat({ id: decodedToken.uid, ...nieuweBuddy }) });
    } catch (err) {
...
export async function handleZetRustperiode(req, res, decodedToken) {
    ...
    try {
        ...
        return res.status(200).json({ success: true, buddy: metDagstaat({ id, ...velden }), meldingen });
    } catch (err) {
