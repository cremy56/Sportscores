...
import { verbeterQuiz } from '../sportbuddy/kennissleutel.js';

const XP_EVENT = 10;
const XP_QUIZ = 15;
const XP_MODULE_AF = 50;
const QUIZ_DAGCAP = 3;
...
// ─── COMPLETE KENNIS (quiz afronden) ──────────────────────────────────────────
// Verbetert de quiz server-side, kent XP toe (15/quiz, cap 3/dag; +50 bij de
// eerste keer dat de module wordt afgerond) en bewaart de voortgang in de buddy.
// Kennis-XP is toegestaan (het beloont wéten, niet gezondheidsgedrag — GDD §7).
export async function handleCompleteKennis(req, res, decodedToken) {
    const { module_id, antwoorden } = req.body;
    try {
        const profiel = await getProfiel(decodedToken.uid);
        if (!TOEGESTANE_ROLLEN.includes(profiel.rol)) {
            return res.status(403).json({ error: 'Verboden' });
        ...
        const uitslag = verbeterQuiz(module_id, antwoorden);
        if (!uitslag) {
            return res.status(400).json({ error: 'Ongeldige quizinzending' });
        }
        ...
        const buddy = { id: buddyDoc.id, ...buddyDoc.data() };

        const vandaag = vandaagBrussel();
        const kennis = { ...(buddy.kennis || {}) };
        const eerder = kennis[module_id] || { afgerond: false, beste: 0, quizzen_vandaag: 0, laatste_dag: null };

        // Dagcap op quiz-XP (over alle modules samen)
        const alleVandaag = Object.values(kennis)
            .filter((m) => m.laatste_dag === vandaag)
            .reduce((som, m) => som + (m.quizzen_vandaag || 0), 0);

        let xp = 0;
        const redenen = [];
        if (uitslag.geslaagd) {
            if (alleVandaag < QUIZ_DAGCAP) {
                xp += XP_QUIZ;
                redenen.push('quiz');
            }
            // Eenmalige module-afronding-bonus
            if (!eerder.afgerond) {
                xp += XP_MODULE_AF;
                redenen.push('module_af');
            }
        }

        const nieuwDag = eerder.laatste_dag === vandaag;
        kennis[module_id] = {
            afgerond: eerder.afgerond || uitslag.geslaagd,
            beste: Math.max(eerder.beste || 0, uitslag.correct),
            quizzen_vandaag: (nieuwDag ? (eerder.quizzen_vandaag || 0) : 0) + 1,
            laatste_dag: vandaag,
        };
        await buddyRef.update({ kennis });

        if (xp > 0) {
            const userRef = db.collection('users').doc(decodedToken.uid);
            await userRef.update({
                xp: FieldValue.increment(xp),
                xp_current_period: FieldValue.increment(xp),
                xp_current_school_year: FieldValue.increment(xp),
            });
            await userRef.collection('xp_transactions').add({
                amount: xp,
                reason: 'sportbuddy_kennis',
                source_id: module_id,
                balance_after: null,
                metadata: { correct: uitslag.correct, totaal: uitslag.totaal, redenen },
                created_at: FieldValue.serverTimestamp(),
            });
        }
        ...
        return res.status(200).json({
            success: true,
            uitslag: { correct: uitslag.correct, totaal: uitslag.totaal, geslaagd: uitslag.geslaagd, juist: uitslag.juist },
            kennis: kennis[module_id],
            beloning: { xp },
            dagcap_bereikt: alleVandaag >= QUIZ_DAGCAP,
        });
    } catch (err) {
        console.error('❌ handleCompleteKennis:', err);
        return res.status(500).json({ error: 'Kennis verwerken mislukt' });
    }
}
